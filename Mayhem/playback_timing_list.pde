...
class playback_timing_list
{
  class playback_timing_entry implements Comparable<playback_timing_entry> 
  {
      ...
      public String note;
      public String FX;      //The timing list can also include an autonomous effect that weill run until something else is set
                             //Things like VU-Meter, Random Strobes, Rainbow scrolling etc...

      
      playback_timing_entry(long  sample_time, int bitmap_x_pos, int brightness, int decay,String FX, String note)
      {
  ...
  void add_entry(long sample_time,int bitmap_x_pos,int brightness,int decay,String FX,String note)
  {
    playback_timing_entry entry = new playback_timing_entry(sample_time, bitmap_x_pos, brightness, decay,FX, note);
  ...
  //This will go through all the entries after ther provided sample time 
  //apply the xpos_offset to the xpos. Could be positive or negative.
  //This goes trogether with expanding/contracting the bitmap.
  void retime_section(long start, int xpos_offset)
  {
    TreeSet<playback_timing_entry> retime_list = new TreeSet<playback_timing_entry>();
    playback_timing_entry entry1 = new playback_timing_entry(start, 0, 0, 0,"", "");
    
    retime_list =  (TreeSet<playback_timing_entry>)timing_list.tailSet(entry1);
    
    // Creating an Iterator
    Iterator iterate;
    iterate = retime_list.iterator();

    // Iterating through the subset
    while (iterate.hasNext()) 
    {
      ((playback_timing_entry)iterate.next()).bitmap_x_pos = ((playback_timing_entry)iterate.next()).bitmap_x_pos+xpos_offset;
    }   
  }
  ...
  playback_timing_entry find_next_entry(long pos)
  {
  }
  
  playback_timing_entry find_previous_entry(long pos)
  {
  }
  
  int get_posx_by_sample_time(long pos)
  {
    ///How do we make sure if we do not hit the sample exactly?
    //Add 1 from pos
    //Find prior entry
    //If timr is less then 1/30 sec use, else discard
    
    //Should we return -2 if there is data in the FX field so that can be used instead?
